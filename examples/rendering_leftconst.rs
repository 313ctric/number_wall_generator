...
fn show_leftconst_sequence<F: Fn(usize) -> i128>(func: F, modulo: i128, left_values: [i128; 2], file_path: &'static str) {
	let height = 1000;
	let width  = 1000;

	let mut holder = left_const_wall::LeftConstWallHolder::new_from_sequence_func(func, left_values, modulo, 0, height, 0, width-1);
...
fn main() {
	show_leftconst_sequence(rook,    2, [0, 1], "./left_const_rook.png");
	show_leftconst_sequence(knight,  2, [0, 1], "./left_const_knight.png");
	show_leftconst_sequence(pagoda,  3, [0, 1], "./left_const_pagoda.png"); // this isn't very good
	show_leftconst_sequence(rueppel, 2, [0, 1], "./left_const_rueppel.png");
	show_leftconst_sequence(zigzag,  3, [0, 1], "./left_const_zigzag.png");
}