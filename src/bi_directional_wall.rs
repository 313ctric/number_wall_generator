...
pub struct BiDirectionalWallHolder {
	pub wall: BiDirectionalWall,
...
impl BiDirectionalWallHolder {
	pub fn new(sequence: Vec<i128>, sequence_start: isize, modulo: i128, top: usize, bottom: usize, left: isize, right: isize) -> Self {
		if right < left {
		...
		Self {
			wall: BiDirectionalWall::new(sequence, sequence_start, modulo),
	...
	pub fn new_from_sequence_func<F>(sequence_func: F, modulo: i128, top: usize, bottom: usize, left: isize, right: isize) -> Self 
		where
			F:  Fn(isize) -> i128
	{
		if right < left {
		...
		if bottom > 1 {
			let sequence_left = left+2-bottom as isize;
			let sequence_right = right-2+bottom as isize;

			let sequence_len = sequence_right-sequence_left+1;
	
			let mut sequence = Vec::with_capacity(sequence_len as usize);
	
			for pos in sequence_left..=sequence_right {
				sequence.push(sequence_func(pos));
			}
	
			Self::new(sequence, sequence_left, modulo, top, bottom, left, right)
		} else {
			Self::new(vec![], 0, modulo, top, bottom, left, right)
		}
	}
	...
	fn calculate_inner(&mut self) -> usize {
		...
		let side_offset: isize = (self.bottom_line - line_num) as isize;
		let line_right = self.right_edge + side_offset;
		let line_left = self.left_edge - side_offset;
	...
	pub fn get_last_line(&mut self) -> Option<&[i128]> {
		...
		if self.last_calculated_line == Some(0) {
			self.temp_line = Some(vec![0; (self.right_edge - self.left_edge + 1) as usize]);
			return Some(self.temp_line.as_ref().unwrap().as_slice());
		}
		if self.last_calculated_line == Some(1) {
			self.temp_line = Some(vec![1; (self.right_edge - self.left_edge + 1) as usize]);
			return Some(self.temp_line.as_ref().unwrap().as_slice());
		}
...
/// A number wall where the wall go into both the positive and negative axes
pub struct BiDirectionalWall {
	lines: VecDeque<BiDirectionalWallLine>,
	modulo: i128,
}
impl BiDirectionalWall {
	pub fn new(mut sequence: Vec<i128>, sequence_start: isize, modulo: i128) -> Self {
		if modulo < 0 {
		...
		if (sequence.len() as isize).checked_add(sequence_start).is_none() {
			panic!("Sequences largest value index is too large")
		}
		...
		let sequence_line = BiDirectionalWallLine {
			values: sequence,
	...
	pub fn calculate_next_line(&mut self, start_position:isize, end_position:isize) -> bool {
		...
		let mut line = BiDirectionalWallLine::new(line_num, start_position, end_position);
	...
	pub fn get_last_line(&self, left: isize, right: isize) -> &[i128] {
		if self.lines.len() == 0 {
		...
		let b_left: usize = (left - b.start_offset) as usize;
		let b_right: usize = (right - b.start_offset) as usize;
...
impl fmt::Debug for BiDirectionalWall {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("BiDirectionalWall")
...
#[derive(Debug)]
pub struct BiDirectionalWallLine {
	pub values: Vec<i128>,
	pub start_offset: isize,
	number: usize,
	keep_for: usize,
}
impl BiDirectionalWallLine {
	/// start and end are both inclusive
	pub fn new(number: usize, start: isize, end: isize) -> Self {
		if end < start {
			panic!("Can't construct line {} with a negative or zero length", number)
		}

		let values = Vec::with_capacity( (end-start+1) as usize ); // create the vector
		Self {
			values,
			start_offset: start,
			number,
	...
	#[inline]
	pub fn get(&self, position: isize) -> i128 {
		...
		if position >= self.start_offset + self.values.len() as isize {
			panic!("Tried to fetch value {} in line {}, which is not valid as line ends at {}", position, self.number, self.start_offset + self.values.len() as isize)
		}
		self.values[(position - self.start_offset) as usize]
	}
